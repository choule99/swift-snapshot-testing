    ...
    func diffHelp(
        ...
        func diffChildren(
            ...
            guard (!lhsMirror.isSingleValueContainer && !rhsMirror.isSingleValueContainer)
                || lhs is any _CustomDiffObject
                || rhs is any _CustomDiffObject else {
                print(
                    _customDump(
                        lhs,
                        name: lhsName,
        ...
        switch (lhs, lhsMirror.displayStyle, rhs, rhsMirror.displayStyle) {
            ...
            case let (lhs as any CustomDumpRepresentable, _, rhs as any CustomDumpRepresentable, _):
                out.write(
                    diffHelp(
                        lhs.customDumpValue,
                        rhs.customDumpValue,
            ...
            case let (lhs as AnyObject, .class?, rhs as AnyObject, .class?):
                ...
                if !tracker.visitedItems.contains(lhsItem), !tracker.visitedItems.contains(rhsItem) {
                    ...
                    var occurrence: UInt {
                        tracker.occurrencePerType[subjectType, default: 0]
                    }
                    if tracker.visitedItems.contains(lhsItem) {
                        var lhsID: String {
                            let id = tracker.idPerItem[lhsItem, default: occurrence]
            ...
            case (_, .dictionary?, _, .dictionary?):
                diffChildren(
                    ...
                    collapseUnchanged: true,
                    areEquivalent: {
                        guard let lhs = $0.value as? (key: AnyHashable, value: Any),
                              let rhs = $1.value as? (key: AnyHashable, value: Any) else {
            ...
            case (_, .optional?, _, .optional?):
                guard let lhsValue = lhsMirror.children.first?.value,
                      let rhsValue = rhsMirror.children.first?.value else {
                    diffEverything()
                    break
                }
...
public extension _CustomDiffObject {
    var _customDiffType: Any.Type? {
        nil
    }
}

public extension _CustomDiffObject where Self: AnyObject {
