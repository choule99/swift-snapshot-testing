...
@discardableResult public func customDump<T>(
    _ value: T,
    name: String? = nil,
    indent: Int = 0,
    maxDepth: Int = .max
...
public extension String {
    ...
    init(customDumping subject: some Any) {
        var dump = ""
...
@discardableResult public func customDump<T>(
    _ value: T,
    to target: inout some TextOutputStream,
    name: String? = nil,
    indent: Int = 0,
    maxDepth: Int = .max
) -> T {
...
@discardableResult func _customDump<T>(
    _ value: T,
    to target: inout some TextOutputStream,
    name: String?,
    nameSuffix: String = ":",
        ...
        func dumpChildren(
            ...
            if !mirror.children.isEmpty {
                if mirror.isSingleValueContainer {
                    var childOut = ""
                    let child = mirror.children[mirror.children.startIndex]
                    customDumpHelp(
                        child.value,
                        to: &childOut,
                        name: child.label,
            ...
            case (_, .dictionary?):
                if mirror.children.isEmpty {
                    ...
                    dumpChildren(
                        ...
                        by: {
                            guard let (lhsKey, _) = $0.value as? (key: AnyHashable, value: Any),
                                  let (rhsKey, _) = $1.value as? (key: AnyHashable, value: Any) else {
                                return false
                            }
