...
@MainActor public func assertInlineSnapshot<Value>(
    ...
    withSnapshotTesting(record: record) {
        ...
        do {
            ...
            guard let difference = snapshotting.diffing.diffV2(expected, actual ?? "")?.0 else {
                return
            }
...
private func writeInlineSnapshots() {
    inlineSnapshotState.withLock { inlineSnapshotState in
        ...
        for (file, snapshots) in inlineSnapshotState {
            ...
            guard let testSource = try? testSource(file: file) else {
...
private final class SnapshotRewriter: SyntaxRewriter {
    ...
    override func visit(_ functionCallExpr: FunctionCallExprSyntax) -> ExprSyntax {
        ...
        guard !snapshots.isEmpty else {
            return super.visit(functionCallExpr)
        }
...
private final class SnapshotVisitor: SyntaxVisitor {
    ...
    override func visit(_ functionCallExpr: FunctionCallExprSyntax) -> SyntaxVisitorContinueKind {
        let location = functionCallExpr.calledExpression
            .endLocation(converter: self.sourceLocationConverter, afterTrailingTrivia: true)
        guard self.functionCallLine == location.line,
              self.functionCallColumn == location.column else {
...
@_spi(Internals) public final class LockIsolated<Value>: @unchecked Sendable {
    ...
    @_spi(Internals) public func withLock<T: Sendable>(
